...
It's interesting to note that you have imported the entire imagery data collection, which consists of all imagery collected over the given time frame and the given area for that sensor. 
...
```javascript
Map.addLayer(LS8_SR1,{},'LS8_SR1',false)
```
...
.filterDate('2015-01-01', '2016-01-01')
  .filterBounds(roi)
  .filterMetadata('CLOUD_COVER', 'less_than', 20).mosaic();
Map.addLayer(LS8_SR2,{},'LS8_SR2',false)
...
Now, we can define our point dataset by specifying the ID of our point data fusion table. In this case, we'll use an example fusion table that has been made public for this tutorial. This fusion table is a binary, presence-absence dataset of points collected in Iowa using high-resolution imagery in GEE. These points indicate areas of forest presence and absence, so we have named the variable "PA". However, your data does not have to be binary.

```javascript
var PA = ee.FeatureCollection('ft:19YknBH-2mw2tl2ZGN6zDYpFaDx_10LP0lEl5r7Oh')
Map.addLayer(PA,{},'PA') 
print(PA,'PA')

```

We can see that our points may not fall within our region of interest so lets edit that geometry so that it does match up. 
<br>
<br>
<br>
