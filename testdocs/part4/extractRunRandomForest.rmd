...
```javascript
var LS8_SR = ee.ImageCollection(LS8_SR)
  .filterDate('2015-01-01', '2016-01-01')
  .filter(ee.Filter.eq('WRS_PATH', 28))
  .filter(ee.Filter.eq('WRS_ROW', 28))
  .filterMetadata('CLOUD_COVER', 'less_than', 20).mosaic();
```
Although we use many filters to narrow down our dataset in this example, you also have the option of running the entire dataset by leaving off the filters:
...
```javascript
var red = LS8_SR.select('B3').rename("red")
var green= LS8_SR.select('B2').rename("green")
var blue = LS8_SR.select('B1').rename("blue")
var nir = LS8_SR.select('B4').rename("nir")
var swir1 = LS8_SR.select('B5').rename("swir1")
var swir2 = LS8_SR.select('B7').rename("swir2")
print(red)
Map.addLayer(red)
print(green)
Map.addLayer(green)
